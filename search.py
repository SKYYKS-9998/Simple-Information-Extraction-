...
class InfoRetrie:
    def __init__(self):
        print("Loading data")
        self.path = 'bbc'      # 数据根路径
        self.dir_list = [self.path]      # 存储根路径下所有目录的目录名
        ...
        self.bag_dict, self.word_count_dict = self.load_bag_vector()
    ...
    def generate_inverse_index(self):
        ...
        for t_index in self.data_dict.keys():
            if len(self.data_dict[t_index]) == 0:
                continue
            # 取所有存在的单词
            words = self.bag_dict[t_index].get_feature_names_out()
                ...
                for i, word in enumerate(words):
                    ...
                    if count_array_dict[t_index][index][i] != 0:
                        ...
                        if len(position_list):
                            result[word].append((t_index + ' ' + str(index), count_array_dict[t_index][index][i],
                                                 position_list))
        return result
    ...
    def do_search(self, search_str):
        ...
        for key, value in result_dict.items():
            search_vec = CountVectorizer(vocabulary=self.bag_dict[value.path].get_feature_names_out()).\
                fit_transform([search_str]).toarray()
            value.similarity = self.get_similarity(search_vec[0],
                                                   self.word_count_dict[value.path][int(value.name)].A[0])
...
class Result:
    ...
    def output(self):
        ...
        print("TXT file name: ", self.name)
        print("Appearance counts: ", self.freq)
